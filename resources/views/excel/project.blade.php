<div class="card rounded-3 mx-auto">
    <div class="card-body d-flex flex-column">
        <h1 class="border border-gray-500 border-active active text-center mb-4">Dự án</h1>
        <div class="table-responsive ">
            <table
                class="table table-striped table-rounded border border-gray-300 table-row-bordered table-row-gray-300">
                <thead>
                    <tr class="fw-semibold fs-4 text-gray-500">
                        <th rowspan="2">Hệ thống</th>
                    ...
                    @foreach ($systems as $system)
                        @foreach ($system->devices as $keyDevice => $device)
                            @foreach ($device->specifications as $keySpecification => $specification)
                                <tr>
                                    @if ($keyDevice == 0 && $keySpecification == 0)
                                        <td class="text-nowrap" rowspan="{{ count($system->specifications) }}">
                                            {{ $system->name }}</td>
                                    @endif
                                    ...
                                    @endif
                                    <td class="text-nowrap"> {{ $specification->id }} -{{ $specification->name }}
                                    </td>
                                    @foreach ($tasks as $date => $taskByDate)
                                        @foreach ($taskByDate->groupBy('shift_id') as $taskByshift)
                                            <td>
